...
class Tests:
    ...
    def Test_3(seq, mod, alpha, subseqsAmount, intervalsAmount):
        ...
        def MakingSubseqs(seq):
            """принимает выборку seq.
        ...
        seqArray = MakingSubseqs(seq)
        hit_Test1 = True
        hit_Test2 = True

        for i in range(len(seqArray)):
            if (Tests.Test_1(seqArray[i], alpha) == False):
                hit_Test1 = False
            if(Tests.Test_2(seqArray[i], mod, alpha, intervalsAmount, False) == False):
                hit_Test2 = False
